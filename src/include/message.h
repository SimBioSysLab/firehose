#ifndef _FIREHOSE_NETWORK_MESSAGE_H_
#define _FIREHOSE_NETWORK_MESSAGE_H_
...
class msg_protobuf_tx : public message_tx {
 ...
 public:
  ...
  virtual uint64_t get_tx_header_len() const { return msg.ByteSizeLong(); }

  virtual void serialize_tx_header(void *dest) {
    msg.SerializeWithCachedSizesToArray(reinterpret_cast<uint8_t *>(dest));
  }
...
class msg_protobuf_rx : public message_rx {
 ...
 public:
  ...
  virtual void body_buf_received(size_t len) { throw "Should not be called"; }
...
class msg_protobuf_rx_with_body : public msg_protobuf_rx<TMsgType, TMsg, TRsp> {
 ...
 public:
  ...
  virtual void body_buf_received(size_t len) {
    if (len != body_len_)
      throw("Message length did not match expected payload size");
  }
};
}
}
