...
void message_sender::start_send(message_tx &req) {
  ...
  conn_->stats.message_sent(pre_header[0] + pre_header[1] + 48);
...
void message_receiver::handle_pre_read(const asio::error_code &error,
  ...
  conn_->stats.message_received(pre_header[0] + pre_header[1] + 48);
...
void message_connection::established() {
  ...
  socket_.set_option(asio::socket_base::receive_buffer_size(firehose::tcp_rcv_buf_size));
  socket_.set_option(asio::socket_base::send_buffer_size(firehose::tcp_snd_buf_size));
